...
const formatStudent = (record) => {
    ...
    return {
        ...
        courses: courses
            .filter(c => c != null)
            .map(c => ({
                _id: c.properties.id || c.properties.course_code,
                courseCode: c.properties.course_code || c.properties.courseCode,
                courseName: c.properties.name || c.properties.courseName // 'name' is what Neo4j uses
            })),
...
const createStudent = async (req, res) => {
    ...
    try {
        ...
        if (courses && courses.length > 0) {
            await session.run(
                `MATCH (s:Student {id: $studentId})
                 UNWIND $courseIds AS cCode
                 MATCH (c:Course) 
                 WHERE c.course_code = cCode OR c.id = cCode
        ...
        console.error("Backend Error:", error); // This helps you see the error in the terminal
